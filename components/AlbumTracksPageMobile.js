...
import AlbumTrackMobile from './AlbumTrackMobile'
  ...
  return (
    ...
    <div className='text-sm mb-5 tracking-tighter text-gray-700'>Explore {album?.data?.title} the {album?.data?.album_type} from {video?.details?.stage_name}</div>
            ...
            <div className='flex mb-5'>
                <div className='w-1/3'>
                    ...
                    {album?.data?.cover && <picture>
                        <img
                            src={!album?.data?.cover ? cover : album?.data?.cover}
                            alt={`${album?.data?.title} poster`}
                            className="h-24 w-full object-cover bg-gray-200"
                        />
                    </picture>}
                </div>
        ...
        {!isLoading && [...Array(albumTracks?.data?.length).keys()].map((albumTrack, i) => (
            <AlbumTrackMobile track={albumTracks?.data[i]} key={i}/>
        ))}
        </div>}
...
export default AlbumTracksPageMobile