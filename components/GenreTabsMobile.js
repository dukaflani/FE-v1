...
const GenreTabsMobile = () => {
  const router = useRouter()
  const { genre } = router.query
  const genrePanel = useRef(null)
  const [activeTab, setActiveTab] = useState('')
  const activeSyles = 'p-1 text-sm bg-gray-600 hover:bg-gray-500 text-white rounded-full cursor-pointer tracking-tighter flex flex-nowrap'
  const regularSyles = 'p-1 text-sm bg-gray-200 hover:bg-gray-300 rounded-full cursor-pointer tracking-tighter flex flex-nowrap'
  ...
  return (
    <div className='fixed top-14 z-30 mb-5 flex w-full bg-white shadow-sm'>
      <div className='flex items-center justify-start mx-2 border-r pr-2'>
        <div className='bg-gray-300 p-1 text-sm'>
          ...
          {/* <button 
            ...
            {!genre && <div ref={genrePanel} className='text-gray-800 flex items-center justify-start flex-1 gap-1 pt-2 pb-2 whitespace-nowrap overflow-x-scroll scrollbar-thin scrollbar-track-transparent scrollbar-thumb-transparent '>
...
export default GenreTabsMobile