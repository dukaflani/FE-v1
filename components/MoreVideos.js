    ...
    <>
        {filteredVideoArr?.map((video, i) => {
            if (filteredVideoArr?.length === i + 1) {
                return (
                    <Link key={i} href='#currentVideo'>
                        <div ref={lastVideoElementRef} onClick={() => handleCurrentVideo(video?.youtube_id, video?.id)} className={video?.id == 1 ? 'hidden' : "flex items-start justify-start space-x-2 mb-5 px-5 cursor-pointer"}>
                ...
                return (
                    <Link key={i} href='#currentVideo'>
                        <div onClick={() => handleCurrentVideo(video?.youtube_id, video?.id)} className={video?.id == 1 ? 'hidden' : "flex items-start justify-start space-x-2 mb-5 px-5 cursor-pointer"}>
                                <div className='relative h-24 w-40'>
                                    {video?.thumbnail && <Image
                                        src={video?.thumbnail}
                                        layout="fill"
