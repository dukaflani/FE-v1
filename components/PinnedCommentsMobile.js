...
import { TrashIcon, BookmarkSlashIcon, PencilSquareIcon } from '@heroicons/react/24/outline'
...
const PinnedCommentsMobile = ({ comment }) => {
  ...
  return (
    <>
        <div className={comment?.is_pinned ? 'flex w-full mb-6 space-x-2 relative' : 'hidden'}>
            ...
            <div className='w-11/12 flex flex-col items-start justify-start pr-10 pt-2'>
                ...
                <Linkify componentDecorator={(decoratedHref, decoratedText, key) => ( <a target="blank" className='text-blue-600 -mb-1 w-56 inline-block overflow-hidden overflow-ellipsis whitespace-nowrap'  href={decoratedHref} key={key}> {decoratedText} </a> )} >
                    <ShowMoreText
                        ...
                        more="Show more"
                        less="Show less"
                        className="content-css text-sm leading-4 tracking-tight text-gray-800 whitespace-nowrap"
            ...
            <div className="absolute top-0 right-0 flex items-start justify-end p-1">
                            ...
                            <Transition
                                ...
                                <div>
                                        {commentOwner == loggedInUser ? (<Menu.Item onClick={handlePinComment} className="text-sm cursor-pointer px-2 py-2 flex items-center justify-start w-full hover:bg-gray-50">
                                                <button>
                                                    <BookmarkSlashIcon className='h-5 w-5 mr-2 ml-1' /> Remove From Top
                                                </button>
                                        </Menu.Item>) : (
...
export default PinnedCommentsMobile